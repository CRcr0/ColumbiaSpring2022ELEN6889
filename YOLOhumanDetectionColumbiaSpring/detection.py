...
import time
import datetime
from utilss.UploadAWS import upload_file, upload_file2
import threading
import boto3
from utilss.ImageDetection import compare_faces
from LambdaFunction.lambda_function import face_analysis
from utilss.email_sender import email_raw
...
def run():
    ...
    # rek = boto3.client('kms', region_name='us-west-2')

    # visual detecting model - yolov5 nano version
    model = torch.hub.load('ultralytics/yolov5', 'yolov5n')

    # denotes whether to record video or detect for person
    isDetected = False
...
def analysis_email(rek, name_vid, name_pic):
    ...
    # violence_prob = 0.8
    if stranger_count > 0:
        response = email_raw('6889proj', name_vid, name_pic, stranger_count, violence_prob)
